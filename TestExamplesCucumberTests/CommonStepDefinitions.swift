    ...
    fileprivate func setup(_ application: XCUIApplication)
    {
        ...
        MatchAll("I launched an App") { (args, userInfo) in
         ...
         MatchAll("^I should see menu with \"([^\\\"]*)\" items$") { (args, userInfo) in
            ...
            let appMenuCount: UInt = UInt(application.menus.count)
         ...
         MatchAll("^I should see screen with \"([^\\\"]*)\" cells$") { (args, userInfo) in
            ...
            let appCellCount: UInt = UInt(application.cells.count)
        ...
        MatchAll("^I see (?:the )?\"([^\\\"]*)\" (button|label|tab|view|field|textView)$") { (args, userInfo) -> Void in
            let label = args?[0]
            let type = args?[1]
